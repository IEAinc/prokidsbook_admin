...
const generateRandomData = (count: number) => {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const randomSubjectRecommendation = storyContents[Math.floor(Math.random() * storyContents.length)];
    ...
    sampleData.push({
      ...
      subjectRecommendation: randomSubjectRecommendation,
...
const GptPrompts:React.FC = () => {
  const navigate = useNavigate();
  const handleRowClick = (row: { id: string | number }) => {
    navigate(`/prompts/gpt/detail/${String(row.id)}`)
  }
  const columns = [
    ...
    { key: 'subjectRecommendation', label: '주제 추천', width: 10, centerType: "text-center" },
    { key: 'gptStory', label: '출력 동화(GPT)', width: 20, centerType: "text-center" },
