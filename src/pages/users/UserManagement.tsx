...
const generateRandomData = (count: number) => {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const randomUsername = `user${Math.floor(Math.random() * 100) + 1}${domains[Math.floor(Math.random() * domains.length)]}` //이메일
    const randomPhone = `010-${Math.floor(Math.random() * 9000) + 1000}-${Math.floor(Math.random() * 9000) + 1000}` //전화번호
    const randomMembership = memberships[Math.floor(Math.random() * memberships.length)] //멤버십
    ...
    sampleData.push({
      id: i.toString(),
      user_id: `user${i}`, //유저 id (상세 페이지 이동용)
...
const UserManagement: React.FC = () => {
  ...
  const handleMakeClick = (row: { user_id: string }) => {
    navigate(`/users/images/${row.user_id}`)
  }
