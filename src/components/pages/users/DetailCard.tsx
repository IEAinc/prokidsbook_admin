/**
 * @file DetailCard.tsx
 * @description 회원 관리 > 회원 상세 페이지 > 회원 상세 카드
 */
...
import ch_img_sm from '../../../assets/images/ch_sm_img.png' /* [삭제 예정] 임시 이미지 */

interface DetailCardProps {
  userId?: string
}

const DetailCard: React.FC<DetailCardProps> = ({ userId }) => {
  /* [삭제 예정] 임시 데이터 */
  const fieldData = {
    id: userId || '1',
    name: '조현지',
    ...
    <div>
        ...
        <div className="grid grid-cols-8 text-m">
          ...
          <div className="p-4 col-span-6 border-b border-gray-200 dark:border-gray-600">
            <input
              ...
              className={`p-2 rounded flex-1 w-full max-w-full focus:outline-gray-400 bg-white dark:bg-[#252731] ${isEdit ? 'border border-gray-400' : 'border-0'
                }`}
              disabled={!isEdit}
            />
          </div>
      ...
      <div className="flex justify-center space-x-2 mt-4">
        ...
        <Btn
          ...
          background="regular"
          onClick={() => {
            setStartDate(startDate) /* 날짜: 닫기 버튼 실행시 리셋 */
            setEndDate(endDate)
            navigate(-1) /* 이전으로 이동 */
          }}
        >
